...
const float convFact[3] = {10/7.7, 0.91, 1};
...
void initialize() {
    ...
    pros::Task screen_task([&]() {
        while (true) {
            ...
            pros::lcd::print(0, "X: %f", chassis.getPose().x*convFact[1]); // x
            pros::lcd::print(1, "Y: %f", chassis.getPose().y*convFact[0]); // y
            pros::lcd::print(2, "Theta: %f", chassis.getPose().theta * convFact[2]); // heading
			pros::lcd::print(3, "ConvFact: %f", convFact);
...
void opcontrol() {
	...
	float startPos[3] = {10, 0, 0};
	chassis.moveToPose(startPos[0] * convFact[0], startPos[1] * convFact[1], startPos[2] * convFact[2], 10000);
